...
namespace LibVLCSharp.Benchmarks
{
    ...
    /// 
    /// </summary>
    public class StreamMediaInputBench
    {
        LibVLC? libvlc;
        ...
        {
            var dir = PlatformHelper.IsX64BitProcess ? "win-x64" : "win-x86";

            var libvlcFolder = Path.Combine(Directory.GetParent(Assembly.GetExecutingAssembly().Location).FullName, Path.Combine("libvlc", dir));

            Core.Initialize(libvlcFolder);
            ...
            media = new Media(libvlc, new StreamMediaInputCurrent(stream), ":no-video", ":no-audio");
        ...
        // how many times a callback is called in 5 seconds?
        ...
        /// <returns></returns>
        [Benchmark]
        public async Task StandardStreamRead()
        {
            _ = mediaplayer?.Play();
        ...
        /// </summary>
        public class StreamMediaInputCurrent : MediaInput
        {
        ...
        private readonly byte[] _readBuffer = new byte[0x4000];
            ...
            public StreamMediaInputCurrent(Stream stream)
            {
